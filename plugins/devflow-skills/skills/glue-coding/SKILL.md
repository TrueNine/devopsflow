...
If the request has unclear domain language, hidden business rules, state transitions, multi-role collaboration, authority questions, policies, aggregates, or read-model ambiguity, use `ddd-event-storming-design` first. Code patterns may be read early as discovery input, but they must not become the final design before the DDD gate is confirmed.
...
2. If `domain_blocked`, return to `ddd-event-storming-design`. Use discovered code only as evidence for questions and candidate language.
...
7. Pass the selected pattern and delta to `implementation-planning` or `ddd-to-tdd-handoff`.
...
2. Use `tdd-skill` characterization tests before changing behavior-preserving code.
3. Search for target patterns elsewhere in the repository or in explicit reference material.
4. If no target pattern exists, pass a target-design task to `implementation-planning` instead of copying the legacy shape.
...
- Do not turn Glue Coding into test avoidance. Behavior changes still need `tdd-skill` unless the task is documentation-only or explicitly non-executable.
