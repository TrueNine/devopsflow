...
describe("MainAgentWriteGuard", () => {
  ...
  describe("shell command blocking", () => {
    it("blocks main agent shell redirection, rm, git add, and pnpm add", () => {
      expect(shouldBlockTool("Bash", { command: "printf hi > README.md" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "rm -rf build" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "git add README.md" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "git push origin feature/demo" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "pnpm add zod" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "pnpm run generate" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "pnpm run build" })).not.toBeNull()
    })
  })

  describe("safe commands", () => {
    it("allows main agent read, status, and test commands", () => {
      expect(shouldBlockTool("Bash", { command: "cat README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "sed -n '1,20p' README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "sed -n 1,20p README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "sed --quiet '1,20p' README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "sed --silent '1,20p' README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "sed 's/a/b/' README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "rg -n DevFlow README.md" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "git status --short" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -c 'print(1)'" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -c 'open(\"README.md\").read()'" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -X utf8 scripts/test-prevent-main-agent-write.py" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -X utf8 scripts/run_design_examples.py" })).toBeUndefined()
      expect(shouldBlockTool("Bash", {
        command: "python3 -X utf8 skills/tdd-skill/scripts/run_protocol_examples.py",
      })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -m unittest" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -m pytest" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -m py_compile scripts/prevent-main-agent-write.py" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "python3 -m json.tool hooks/hooks.codex.json" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "pnpm test" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "pnpm run test" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "pnpm run typecheck" })).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "pnpm run lint" })).toBeUndefined()
    })
  })

  describe("unsafe commands", () => {
    it("blocks main agent mutating sed and unsafe python commands", () => {
      expect(shouldBlockTool("Bash", { command: "sed -i 's/a/b/' README.md" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "sed --in-place 's/a/b/' README.md" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "python3 -c 'open(\"README.md\", \"w\").write(\"x\")'" })).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "python3 scripts/update_readme.py" })).not.toBeNull()
    })
  })

  describe("proxy escalation", () => {
    it("escalates proxy bypass attempts", () => {
      const decision = shouldBlockTool("Bash", { command: "rtk proxy sed -n '1,20p' README.md" }, "worker-1")
      expect(decision).not.toBeNull()
      expect(decision!.escalation).toBe(true)
      expect(decision!.reason).toInclude("二级警告")

      const decision2 = shouldBlockTool("Bash", { command: "proxy python3 -c 'print(1)'" }, "worker-1")
  ...
  describe("subagent sessions", () => {
    it("blocks git push even for registered subagent", () => {
      ...
      expect(shouldBlockTool("Bash", { command: "git push -u origin codex/task" }, "worker-1")).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "command git push origin codex/task" }, "worker-1")).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "git -C ../repo push origin codex/task" }, "worker-1")).not.toBeNull()
    })

    it("allows registered subagent write", () => {
      startSubagent("worker-1")
      expect(shouldBlockTool("Write", {}, "worker-1")).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "git add README.md" }, "worker-1")).toBeUndefined()
    })
    ...
    it("blocks write without session id", () => {
      expect(shouldBlockTool("Write", {})).not.toBeNull()
      expect(shouldBlockTool("Bash", { command: "touch marker" })).not.toBeNull()
    })
  ...
  describe("protected branch write for registered subagent", () => {
    it("blocks registered subagent write on protected branch via workdir", () => {
      ...
      expect(shouldBlockTool("Bash", { command: "touch marker", workdir: root }, "worker-1")).toBeUndefined()
      expect(shouldBlockTool("Bash", { command: "touch marker", workdir: nested }, "worker-1")).toBeUndefined()

      switchBranch(root, "dev")
      const decision = shouldBlockTool("Bash", { command: "touch marker", workdir: root }, "worker-1")
      expect(decision).not.toBeNull()
  ...
  describe("hook manifest", () => {
    it("hook manifest uses codex plugin root variable", () => {
      ...
      const manifestPath = join(import.meta.dir, "..", "hooks", "hooks.codex.json")
