...
- Use `$df-jimmer-kotlin` when changing Kotlin source sets, KSP, `df-jimmer-sql-kotlin`, Kotlin DSL, or `KSqlClient`.
- Use `$df-jimmer-java` when changing Java source sets, APT, Maven/Gradle annotation processing, Java DSL, or `JSqlClient`.
