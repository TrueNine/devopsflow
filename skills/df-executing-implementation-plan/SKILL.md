...
4. Record evidence:
   ...
   - result summary
5. If tests fail unexpectedly, switch to `df-systematic-debugging`.
6. If the next step is TDD behavior work, use `df-tdd-skill`.
7. If the task touches Spring Web boundaries, use `df-spring-web-boundaries`.
...
- Do not claim completion until `df-verification-before-completion` has checked the whole result.
