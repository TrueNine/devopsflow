...
- After root cause is identified, use `df-tdd-skill` for the regression test and fix if behavior changes.
- If the root cause shows a bad plan, return to `df-implementation-planning`.
- If the failure touches Spring Web contracts, use `df-spring-web-boundaries`.
- Before completion, use `df-verification-before-completion`.
