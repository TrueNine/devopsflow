...
Use this skill to author and maintain `.df-drawio.svg` / diagrams.net diagrams. Prefer a single editable SVG file that renders directly in Markdown and still opens cleanly in diagrams.net, instead of screenshots or Mermaid-only output.

## When To Use

- Use this skill when the user explicitly asks for `.df-drawio.svg`, `.df-drawio`, draw.io, diagrams.net, mxGraphModel, architecture diagrams, flowcharts, deployment diagrams, or data-flow diagrams.
- For new committed documentation diagrams, default to the `.df-drawio.svg` suffix.
- If the repository already has `.df-drawio` files, maintain them in place and reuse the existing XML indentation, colors, page size, and layout style.
- Mermaid is often faster for lightweight explanatory sketches. Use `.df-drawio.svg` when the diagram should render directly in Markdown, be edited visually, committed to documentation, or maintained over time.
- Do not treat `.df-drawio.svg` as a hand-written SVG or binary image. It is SVG XML with embedded diagrams.net data and should be edited structurally.

## File Structure

Prefer editable `.df-drawio.svg` files exported from diagrams.net with embedded diagram data. The SVG must remain valid XML and preserve the diagrams.net metadata needed for visual editing.

When the project still uses plain `.df-drawio` sources, prefer full uncompressed XML:
...
- Do not generate compressed diagram payloads unless the project already uses compressed `.df-drawio` or `.df-drawio.svg` files and the user asks to preserve that style.
...
- Read the full `.df-drawio.svg` or `.df-drawio` file first. Do not edit by blind text replacement.
...
- Run an XML well-formedness check such as `xmllint --noout file.df-drawio.svg`; if `xmllint` is unavailable, use any available XML parser.
...
- Without an explicit separate source/export convention, commit only the `.df-drawio.svg` file. If the repository requires separate exported PNG/SVG companions, generate them with the existing project script.
