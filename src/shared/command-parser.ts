export const GIT_GLOBAL_OPTIONS_WITH_VALUE = new Set(["-C", "-c", "--git-dir", "--work-tree", "--namespace"])
...
export const PACKAGE_MANAGERS = new Set(["npm", "pnpm", "yarn", "bun", "pip", "pip3", "uv", "cargo", "go"])
...
function shellTokenize(command: string): string[] {
  ...
  while (i < command.length) {
    ...
    let token = ""
    while (i < command.length && !/\s/.test(command[i]) && command[i] !== "'" && command[i] !== '"') {
      token += command[i]
      i++
    }
    tokens.push(token)
...
export function stripLauncherPrefix(tokens: string[]): string[] {
  ...
  while (changed) {
    ...
    while (normalized.length && ["command", "builtin", "exec", "env"].includes(normalized[0] as string)) {
      normalized.shift()
      changed = true
    }
  }
...
export function gitSubcommand(args: string[]): string | undefined {
  ...
  while (index < args.length) {
    ...
    if (GIT_GLOBAL_OPTIONS_WITH_VALUE.values().some((opt) => token.startsWith(`${opt}=`))) {
      index += 1
      continue
    }
...
export function isPackageCommandWrites(tokens: string[]): boolean {
  ...
  if (["npm", "pnpm", "yarn", "bun"].includes(manager)) {
    return ["add", "build", "generate", "codegen", "install", "i", "remove", "rm", "run", "update", "upgrade"].includes(subcommand)
  }
...
export function isPackageCommandWritesRestricted(tokens: string[]): boolean {
  ...
  if (["npm", "pnpm", "yarn", "bun"].includes(manager)) {
    return ["add", "install", "i", "remove", "rm", "update", "upgrade"].includes(subcommand)
  }
  if (["pip", "pip3", "uv"].includes(manager)) {
    return ["install", "add", "remove", "sync"].includes(subcommand)
  }
...
export function isPythonSafeTestCommand(args: string[]): boolean {
  ...
  if (filtered[0] === "-m" && filtered.length >= 2) {
    return ["pytest", "unittest", "py_compile", "json.tool"].includes(filtered[1])
  }
...
export function pythonWriteReason(args: string[]): string | undefined {
  ...
  if (["write", "update", "generate", "codegen", "modify", "migrate"].some((m) => script.includes(m))) {
...
export function pythonInlineCodeWrites(code: string): boolean {
  ...
  return [".write(", ".write_text(", ".write_bytes(", "os.remove(", "shutil."].some((m) => code.includes(m))
}
...
export function stripPythonRuntimeOptions(args: string[]): string[] {
  ...
  while (index < filtered.length) {
    ...
    if ((token.startsWith("-X") || token.startsWith("-W")) && token.length > 2) {
      index += 1
      continue
    }
...
export function isTestCommand(tokens: string[]): boolean {
  ...
  if (["mvn", "gradle", "./gradlew"].includes(command)) {
    return args.some((arg) => arg.toLowerCase().includes("test") || arg.toLowerCase().includes("check"))
  }

  if (["python", "python3"].includes(command)) return isPythonSafeTestCommand(args)
